 ...
 *   creative it supplies. Priced per week, paid in points.
 *
 *   A READER becomes a member — one fixed amount a month, no creative, no
 *   dates, and a face on the public wall.
 *
 ...
 * on both doors is read live — the cheapest open placement out of Neon, the
 * membership price out of the Ghost tier, the member count off the wall. Where
 * a number cannot be read it is simply absent (CLAUDE.md §4: a figure here is
 * either real or it is not shown). Neither door ever renders a guess.
 */
export async function TwoDoors() {
  const creatorId = await getCreatorId();
  const [open, memberCount, tier] = await Promise.all([
  ...
  const tierPrice = tier && tier.monthlyPrice != null ? formatTierPrice(tier) : null;
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      <Door
        ...
        meta={
          cheapest !== null
            ? `${open.length} open now · from ${cheapest.toLocaleString()} points a week`
      ...
      <Door
        ...
        body="One fixed amount a month, no ad to design and no dates to pick. You get a face on the public sponsor wall — with your own line and your own link — and a paid membership on the blog."
        meta={
          [
            tierPrice ? `${tierPrice} a month` : null,
            memberCount > 0
              ? `${memberCount} ${memberCount === 1 ? 'member' : 'members'} on the wall`
              : null
          ]
            .filter(Boolean)
            .join(' · ') || 'Cancel any time'
        }
        href="/members"
        cta="Join the wall"
      />
