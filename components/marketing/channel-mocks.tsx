...
/**
 * One placeholder mock per channel — the shape of a placement, never a real
 * logo or an invented number (CLAUDE.md §4). Shared by two consumers: the
 * tabbed `AdPreviewShowcase` on /placements, and the auto-cycling
 * `HeroShowcase` on the homepage. They differ only in how they choose which
 * mock is on screen, so the mocks themselves live here rather than inside
 * either one.
 */
  ...
  return (
    <div className="win win-phone mx-auto">
      <div className="win__body flex h-full flex-col justify-center gap-3 p-5">
        ...
        <p
          className={cn('font-label text-2xs uppercase tracking-slate text-subtle', styles.fadeInUp)}
        >
...
export const CHANNEL_MOCKS: Record<ChannelKey, React.ComponentType> = {
  blog: BlogMock,
  youtube: YoutubeMock,
